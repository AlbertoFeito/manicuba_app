...
class _LicenciaScreenState extends State<LicenciaScreen> {
  ...
  Future<void> _activar() async {
    ...
    if (ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('¡Licencia activada! Gracias ${config.emoji}')),
      );
  ...
  Future<void> _compartir() async {
    final config = AppConfig.instance.current;
    await Share.share(
      'Hola, quiero activar ${config.appName} ${config.emoji}. Mi código de '
      'equipo es: $_codigoFmt',
      subject: 'Licencia ${config.appName}',
    );
  }
