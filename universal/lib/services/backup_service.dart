...
class BackupService {
  ...
  static Future<String?> createBackupFile({String? storeName}) async {
    try {
      ...
      final filename = _backupFilename(storeName ?? AppConfig.instance.current.appName);
  ...
  static Future<void> shareBackup({String? storeName}) async {
    try {
      final nombre = storeName ?? AppConfig.instance.current.appName;
      final json = await exportData();
      final filename = _backupFilename(nombre);
      ...
      if (_esMovil) {
        ...
        await Share.shareXFiles(
          [XFile(file.path)],
          subject: 'Copia de seguridad de $nombre',
          text: 'Copia de seguridad de $nombre',
        );
      } else {
  ...
  /// Valida que un JSON sea un backup válido de esta app.
